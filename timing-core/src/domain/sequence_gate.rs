...
//! # Restart semantics
//!
//! On startup, [`crate::services::seed_from_log`] walks the persisted event
//! log and rebuilds the per-`(producer_id, detector_id)` high-water map from
//! every stored Detection. After seeding, a producer reconnecting with the
//! same `producer_id` after a node restart sees the same idempotence
//! guarantee it had before the restart: a previously-acknowledged sequence
//! number is rejected as a duplicate, not silently re-persisted. Seeding
//! lives in the service layer because it reads through an outbound port;
//! this module stays free of port types.
...
mod tests {
    use super::*;
    use crate::testing::detection as det;
