...
1. Satisfy the contracts for every role it plays. Most contract crates live in this workspace; `plugin-api` is listed for completeness but is not yet specified.
   ...
   - For a **transport-binding ingest adapter**: the [`EventIngestPort`](../timing-core/src/ports/outbound/ingest.rs) trait in `timing_core::ports::outbound`. Server-side framing and handshake are reusable via [`frame-codec`](../frame-codec) + [`ingest-protocol`](../ingest-protocol); concrete adapters consume both.
...
- **Transport Binding.** Each binding's listener/client lifecycle, reconnect semantics, and error reporting match the [`EventIngestPort`](../timing-core/src/ports/outbound/ingest.rs) contract in `timing_core::ports::outbound`. Bindings are tested independently of higher layers.
