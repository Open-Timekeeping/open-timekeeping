...
Server-side port traits, all inside [`timing-core`](../timing-core). They split by **who implements them**, not by which way data travels: the core implements every inbound port, adapters implement every outbound port.

- **`timing_core::ports::inbound::EventAppendPort`**, the write-side entry point into the ingest pipeline. Implemented by `timing-core::services::EventIngestService`; the runtime's listener loop holds `Arc<dyn EventAppendPort>` and never names the concrete service. Its error vocabulary is deliberately storage-agnostic, so a backend failure never reaches the runtime as a storage type.
- **`timing_core::ports::outbound::EventIngestPort`**, the common abstraction every transport-binding ingest adapter implements (listener accept loop, session lifecycle, error vocabulary). Implementers: `adapter-ingest-tcp`, `adapter-ingest-unix-socket`. Outbound despite carrying producer traffic inward: the adapter implements it and the runtime calls it. Its error vocabulary is transport-neutral, so an adapter over serial, USB-CDC, or an in-process link is never forced to express failures in socket terms.
...
| `timing-core` | The hexagon. Three sibling modules: **`domain/`** (`Crossing`, `CrossingProcessor`, `SequenceGate`, `ProcessorConfig`; pure, names no port type), **`ports/inbound/`** (`EventAppendPort`, `EventQueryPort` — implemented by the core) + **`ports/outbound/`** (`EventLog`, `EventIngestPort`, `IngestMetrics` — implemented by adapters), **`services/`** (`EventIngestService`, which implements both inbound ports and takes the outbound ports as constructor arguments; plus `seed_from_log`, which restores gate state from the log at startup). Adapter crates and `conformance` depend on `timing-core` for the port types only; a per-crate `clippy.toml` denies reaching into `domain` or `services`, and a CI job fails the build if a dependent ships without that fence. The composition root (`timing-node`) is the only crate that touches every layer. |
